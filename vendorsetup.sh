...
if [ -f "$(gettop)/bootable/recovery/orangefox.cpp" ]; then
	echo -e "\x1b[96m[INFO]: Setting up OrangeFox build vars for everpal...\x1b[m"
	if [ "$1" = "$FDEVICE" ] || [  "$FOX_BUILD_DEVICE" = "$FDEVICE" ]; then
		...
		export TARGET_DEVICE_ALT="evergo, opal , evergreen , everpal"
		...
		export FOX_USE_SPECIFIC_MAGISK_ZIP="$(gettop)/device/xiaomi/everpal/Magisk/Magisk.zip"

		export BUNDLED_MAGISK_VER="$(wget -qO- "https://ghps.cc/https://github.com/topjohnwu/Magisk/releases" | grep -Eo "Magisk v.*" | cut -d'<' -f1 |  awk '{print $2}' | head -1)"
		export BUNDLED_MAGISK_SUM="$(curl -sL "https://ghps.cc/https://github.com/topjohnwu/Magisk/releases/download/${BUNDLED_MAGISK_VER}/Magisk-${BUNDLED_MAGISK_VER}.apk"  | tee mgsk | sha256sum | awk '{print $1}' && rm mgsk)" # Sha256 sum of the prebuilt magisk
		...
		then
				...
				then
						...
						curl -L --progress-bar "https://ghps.cc/https://github.com/topjohnwu/Magisk/releases/download/${BUNDLED_MAGISK_VER}/Magisk-${BUNDLED_MAGISK_VER}.apk" -o "${FOX_USE_SPECIFIC_MAGISK_ZIP}"
